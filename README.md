...
**Parameters:**
- **`pickle_path`** — Path to `.edges.pickle` file generated by `splitpea run`.
...
**Parameters:**
- **`pickle_path`** — Path to `.edges.pickle` file for the rewired network.
- **`networkx`** — NetworkX object of the rewired network. Must provide either `pickle_path` or `networkx`.
...
- **`neg_networkx`** *(networkx.Graph, optional)* — NetworkX object of consensus negative graph. 
- **`pos_networkx`** *(networkx.Graph, optional)* — NetworkX object of consensus positive graph. 
