...
def run(
    ...
    if differential_format.lower() == "suppa2":
        if not (isinstance(in_file, (list, tuple)) and len(in_file) == 2):
            raise ValueError(
                "For SUPPA2 input format, in_file must be a list or tuple of 2 file paths: [psivec_file, dpsi_file]."
            )
        ...
        if map_path is None:
            ...
            map_path = str(file)
        fileExists(map_path)
        in_file = parse_suppa2(
            psivec_file, dpsi_file, map_path, species=species, verbose=verbose
        )
        ...
        if type(in_file) != str:
            if len(in_file) > 1:
                raise ValueError(
                    "For differential_formats other than SUPPA2, only a single input file is allowed."
                )
            in_file = in_file[0]
        if index is None:
            index = 0
    ...
    if (
        ppif is None
        or ddif is None
        or entrezpfamf is None
        or pfamcoordsf is None
        or tbf is None
    ):
        if species.lower() == "mouse":
            ...
            if entrezpfamf is None:
                entrezpfamf = str(
                    pkg_resources.files(mouse_ref).joinpath("mouse_entrez_pfam.txt")
                )
            if pfamcoordsf is None:
                pfamcoordsf = str(
                    pkg_resources.files(mouse_ref).joinpath(
                        "mouse_pfam_genome_coords_sorted.txt.gz"
                    )
                )
            if tbf is None:
                tbf = str(
            ...
            if entrezpfamf is None:
                entrezpfamf = str(
                    pkg_resources.files(reference).joinpath("human_entrez_pfam.txt")
                )
            if pfamcoordsf is None:
                pfamcoordsf = str(
                    pkg_resources.files(reference).joinpath(
                        "human_pfam_genome_coords_sorted.txt.gz"
                    )
                )
            if tbf is None:
                tbf = str(
    ...
    logger.info(
        "# proteins: %i, # interactions: %i", len(all_ppis), all_ppis.number_of_edges()
    )
    ...
    logger.info(
        "# nodes: %i, # edges: %i", len(diff_splice_g), diff_splice_g.number_of_edges()
    )
    ...
    with open(out_file_prefix + ".edges.dat", "w") as out:
        ...
        for gi, gj in nx.edges(diff_splice_g):
            ppi_ddi_out = [
                gi,
                gj,
                str(diff_splice_g[gi][gj]["weight"]),
    ...
    if edge_stats_file is not None:
        ...
        with open(edge_stats_file, "a") as f:
            ...
            f.write(
                f"{out_file_prefix}\t{stats['gain']}\t{stats['loss']}\t{stats['chaos']}\n"
            )
    ...
    if plot_net == True:
        ...
        plot_rewired_network(
            diff_splice_g,
            with_labels=True,
            pdf_path=out_file_prefix + "_network_plot.pdf",
        )
    if gephi_tsv == True:
        ...
        plot_rewired_network(
            ...
            with_labels=True,
            plot_matplot=False,
            gephi_path=out_file_prefix + "_gephi.tsv",
        )
    if cytoscape_gml == True:
        ...
        plot_rewired_network(
            ...
            with_labels=True,
            plot_matplot=False,
            cytoscape_path=out_file_prefix + "_cyto.gml",
        )
    ...
    if diff_splice_g.number_of_nodes() == 0 or diff_splice_g.number_of_edges() == 0:
        print(
            "Warning: no edges found. Check the gene identifiers (most supported is Ensembl) "
...
def plot(
    ...
    if symbol:
        ...
        for node in G.nodes():
            ...
            node_str = str(node)
            s = (
                entrez_map.get(node_str)
                or ensembl_map.get(node_str)
                or uniprot_map.get(node_str)
            )
...
def main():
    ...
    rewire_p = subparsers.add_parser("run", help="calculate and output rewired network")
    ...
    rewire_p.add_argument(
        "out_file_prefix", default=None, help="Prefix for output files."
    )
    rewire_p.add_argument(
        "--differential_format",
        type=str,
        choices=["sample_specific", "suppa2", "rmats"],
    ...
    rewire_p.add_argument(
        "--dpsi_cut", type=float, default=0.05, help="Delta PSI cutoff (default: 0.05)."
    )
    rewire_p.add_argument(
        "--sigscore_cut",
        type=float,
    ...
    rewire_p.add_argument(
        "--ppif", type=str, default=None, help="Protein-protein interaction file path."
    )
    rewire_p.add_argument(
        "--ddif", type=str, default=None, help="Domain-domain interaction file path."
    )
    rewire_p.add_argument(
        "--entrezpfamf",
        type=str,
    ...
    rewire_p.add_argument("--tbf", type=str, default=None, help="Tabix file path.")
    rewire_p.add_argument(
    ...
    plot_p.add_argument(
        "--symbol", type=bool, default=True, help="Replace node id with gene symbol"
    )
    plot_p.add_argument(
        "--map_path",
        type=str,
    ...
    delta_p.add_argument(
        "bg_file", type=str, help="Path to the raw background spliced exon file"
    )
    delta_p.add_argument(
        "target_file", type=str, help="Path to the compare spliced exon file"
    )
    delta_p.add_argument(
        "outdir", type=str, help="Directory where outputs will be saved"
    )
    ...
    pooled_p.add_argument(
        "compare_path", type=str, help="Path to your compare.txt output file"
    )
    pooled_p.add_argument(
        "out_psi_dir",
        type=str,
